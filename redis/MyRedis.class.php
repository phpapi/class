...
class MyRedis
{
     ...
     * @param int $post
     */
    private function __construct($instance = 'REDIS_DEFAULT')
    {
        try {
            ...
            if ( !class_exists ( 'redis' )) {
                return "Unable to load class: redis ";
            }
            $this->redis = new Redis();
            $this->redis->connect($conf['host'],$conf['port']);
        } catch (Exception $e) {
            halt("redis_connection define wrong:" . $conf['host'] . "," . $conf['port']);
        }
        return $this->redis;
    }

    public static function instance($instance = 'REDIS_DEFAULT')
    {
